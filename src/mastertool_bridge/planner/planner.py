...
# A task do Perfil de Projeto, tal como vem no template qualificado. O nome é
...
def _node_key(family: str, name: str) -> str:
    ...
    `ValidationResult.creation_order` já usa (`duts:ST_Exemplo`)."""
    return family + ":" + name
...
def _check_template(spec: dict, expected_template: Any,
    ...
    não é provar: um plano gerado contra um template e executado sobre outro
...
def _build_authoring_plan(spec: Any, expected_template: Any,
    ...
    # TASK QUE JA EXISTE NAO E CRIADA. `MainTask` vem no template, e
