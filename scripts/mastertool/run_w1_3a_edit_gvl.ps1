<#
.SYNOPSIS
    Host supervisionado de W1.3A -- editar o documento textual_declaration de
    UMA GVL ja existente (GVL_AI_TESTE) e persistir por save_as.
...
.DESCRIPTION
    FAIL-CLOSED. Tres modos MUTUAMENTE EXCLUSIVOS; o default nao muta nada:
        ...
        -PreflightOnly   abre o MasterTool e roda o probe 31 (preflight),
                         somente leitura. Prova que o texto atual e o inicial
        -ExecuteMutation validacao + preflight + probe 32 (mutacao) + probe 31
                         em modo postsave sobre o arquivo salvo

    O probe 32 e alcancavel APENAS pelo ramo -ExecuteMutation.
...
Write-Section "W1.3A -- editar GVL_AI_TESTE -- modo '$Mode'"
...
if ($planData.phase -ne 'W1_3A_EDIT_GVL') { Fail "phase inesperada: $($planData.phase)" }
if ($planData.target_name -ne 'GVL_AI_TESTE') { Fail "target_name inesperado" }
...
$probe31 = Join-Path $RepoRoot 'scripts\mastertool\probes\31_verify_gvl_edit_w1_3a_readonly.py'
if (-not (Test-Path -LiteralPath $probe31)) { Fail "Probe 31 nao encontrado" }
...
$preflightCompletion = Join-Path $preflightDir 'w1-3a-preflight-completion.json'
$obs1 = Invoke-MasterTool -ProjectPath $copyPath -ProbePath $probe31 `
...
if ($null -eq $preflight) { Fail 'w1-3a-preflight-completion.json ausente. Sem veredito.' }
...
if ($Mode -eq 'PreflightOnly') {
    Write-Host 'Texto inicial confirmado, GVL_AI_TESTE unica no container.'
...
$probe32 = Join-Path $RepoRoot 'scripts\mastertool\probes\32_edit_gvl_w1_3a.py'
if (-not (Test-Path -LiteralPath $probe32)) { Fail "Probe 32 nao encontrado" }
...
$obs2 = Invoke-MasterTool -ProjectPath $copyPath -ProbePath $probe32 `
    -ScriptArgs ("--plan=" + $Plan) -StageDir $mutationDir -Label 'MUTACAO (probe 32)' `
...
Write-Host "[INFO] status do probe 32: $($completion.status)"
...
$postsaveCompletion = Join-Path $postsaveDir 'w1-3a-postsave-completion.json'
$baselineTree = Join-Path $preflightDir 'w1-3a-preflight-tree.json'
$obs3 = Invoke-MasterTool -ProjectPath $outputPath -ProbePath $probe31 `
...
if ($null -eq $postsave) { Fail 'w1-3a-postsave-completion.json ausente.' }
...
Write-Section "VEREDITO DA SESSAO W1.3A: $verdict"
