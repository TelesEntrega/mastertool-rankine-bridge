...
# -----------------------------------------------------------------------------
# R3.1A-3-FIX-2 (docs/79): UMA enumeracao por no TAMBEM COM VISITANTE
#
# O teste acima existia e passava -- ele varre SEM visitante, que e o caso do
# probe 21. O defeito da run-065 vivia no outro caminho: com visitante, o probe
# 49 chamava `get_children()` de dentro dele, e o scanner chamava de novo no
# mesmo proxy logo em seguida. 49 nos viraram 27.
# -----------------------------------------------------------------------------

def test_get_children_chamado_uma_vez_por_no_MESMO_COM_VISITANTE():
    """A travessia tem UM dono. O visitante observa o no; nao o enumera."""
    child = FakeNode(name="Device")
    project = FakeProject(children=[child])

    vistos = []

    def visitante(proxy, record):
        vistos.append(record["node_id"])
        return {"ok": True}

    scanner_mod.ReadOnlyProjectScanner().scan(project, visitor=visitante)

    assert vistos, "o visitante precisa ter rodado, senao o teste nao mede nada"
    assert project.get_children_call_count == 1
    assert child.get_children_call_count == 1


class _NoQueMorreNaSegundaChamada(FakeNode):
    """Coleção que morre na SEGUNDA enumeração do mesmo nó.

    Conta as INVOCACOES, e nao os sucessos: `FakeNode.get_children` so
    incrementa o contador dele quando a chamada da certo, entao usar aquele
    contador aqui mediria o oposto do que o teste quer.
    """

    def __init__(self, *args, **kwargs):
        FakeNode.__init__(self, *args, **kwargs)
        self.invocacoes = 0

    def get_children(self, recursive):
        self.invocacoes += 1
        if self.invocacoes > 1:
            raise RuntimeError("The project handle 0 is invalid.")
        return FakeNode.get_children(self, recursive)


def test_segunda_enumeracao_do_mesmo_no_QUEBRARIA_a_varredura():
    """Adversarial: com UMA chamada por nó, o nó frágil sobrevive.

    Reproduz a forma da falha de campo -- `The project handle 0 is invalid` a
    partir do `MainTask`. Se alguém reintroduzir a enumeração dupla, este teste
    cai junto com o controle positivo abaixo.
    """
    fragil = _NoQueMorreNaSegundaChamada(name="MainTask",
                                         children=[FakeNode(name="MainPrg")])
    depois = FakeNode(name="UserGVLs")
    project = FakeProject(children=[fragil, depois])

    def visitante(proxy, record):
        # Um visitante correto NAO toca a colecao. Se este corpo chamasse
        # `proxy.get_children(...)`, o no fragil morreria e os irmaos
        # seguintes viriam junto -- que e exatamente a cascata da run-065.
        return {"kind": "observado"}

    resultado = scanner_mod.ReadOnlyProjectScanner().scan(project,
                                                          visitor=visitante)

    assert fragil.invocacoes == 1
    assert resultado["statistics"]["collection_errors"] == 0
    assert resultado["statistics"]["scan_complete"] is True
    # o irmao POSTERIOR ao no fragil precisa ter sido alcancado: era ele que a
    # cascata levava junto.
    assert depois.get_children_call_count == 1


def test_CONTROLE_POSITIVO_visitante_que_enumera_reproduz_a_cascata():
    """A variante B, offline. Sem este teste, o de cima poderia estar passando
    por ser insensivel -- um fixture que nunca falharia mediria nada.

    Aqui o visitante FAZ o que o probe 49 fazia: chama `get_children()` no
    proxy. O no morre na chamada do scanner, e o irmao seguinte vem junto. E a
    forma da run-065, em miniatura: 27 de 49.
    """
    ...
    def visitante_que_enumera(proxy, record):
        try:
            proxy.get_children(False)
        except Exception:                                          # noqa: BLE001
            return {"membros": []}      # a ausencia FABRICADA, tal como era
        return {"membros": []}

    resultado = scanner_mod.ReadOnlyProjectScanner().scan(
        project, visitor=visitante_que_enumera)

    assert fragil.invocacoes == 2, (
        "o visitante enumerou e o scanner enumerou: duas chamadas no mesmo no")
    assert resultado["statistics"]["collection_errors"] >= 1
    assert resultado["statistics"]["partial_nodes"] >= 1
    assert resultado["statistics"]["traversal_finished"] is True
    assert resultado["statistics"]["scan_complete"] is False


# -----------------------------------------------------------------------------
# R3.1A-3-FIX-2: `scan_complete` e CONJUNCAO, nao sinonimo de "nao abortou"
# -----------------------------------------------------------------------------

def test_traversal_finished_e_scan_complete_sao_campos_DISTINTOS():
    project = FakeProject(children=[FakeNode(name="Device")])
    stats = scanner_mod.ReadOnlyProjectScanner().scan(project)["statistics"]
    assert stats["traversal_finished"] is True
    assert stats["scan_complete"] is True


def test_scan_complete_e_FALSO_com_erro_de_colecao_ainda_que_termine():
    """A run-065: o laco terminou normalmente, com 14 nos parciais, e o campo
    saiu `True`. Nao pode mais."""
    quebrado = FakeNode(name="MainTask",
                        get_children_raises=RuntimeError(
                            "The project handle 0 is invalid."))
    project = FakeProject(children=[quebrado])

    stats = scanner_mod.ReadOnlyProjectScanner().scan(project)["statistics"]

    assert stats["traversal_finished"] is True, (
        "o laco terminou: e esse o fato que o campo antigo media")
    assert stats["collection_errors"] >= 1
    assert stats["partial_nodes"] >= 1
    assert stats["scan_complete"] is False


def test_scan_complete_e_FALSO_quando_a_raiz_nao_confirma():
    """Uma raiz cuja colecao falhou produz arvore de um no so, que passaria por
    todos os contadores zerados -- foi o desfecho da run-049."""
    project = FakeProject(get_children_raises=RuntimeError("sem handle"))
    stats = scanner_mod.ReadOnlyProjectScanner().scan(project)["statistics"]
    assert stats["traversal_finished"] is True
    assert stats["scan_complete"] is False


def test_scan_complete_e_FALSO_quando_um_limite_e_atingido():
    """Varredura truncada terminou normalmente e NAO percorreu a arvore."""
    neto = FakeNode(name="Neto")
    filho = FakeNode(name="Filho", children=[neto])
    project = FakeProject(children=[filho])
    resultado = scanner_mod.ReadOnlyProjectScanner(max_depth=1).scan(project)
    assert resultado["limits"]["max_depth_reached"] is True
    assert resultado["statistics"]["traversal_finished"] is True
    assert resultado["statistics"]["scan_complete"] is False
...
# =============================================================================
# VISITANTE (contrato docs/59) — a rota unica de navegacao serve a inspecao
# =============================================================================

def _arvore_de_tres():
    """GUIDs FIXOS: o padrao do fixture e um contador por instancia, e duas
    arvores construidas em sequencia nunca seriam comparaveis byte a byte —
    o teste de equivalencia mediria o contador, nao o visitante."""
    return FakeProject(children=[
        FakeNode(name="A", type_guid="tg-a", object_guid="og-a",
                 children=[FakeNode(name="A1", type_guid="tg-a1",
                                    object_guid="og-a1")]),
        FakeNode(name="B", type_guid="tg-b", object_guid="og-b"),
    ])


def test_sem_visitante_a_saida_e_a_de_sempre():
    """`visitor=None` tem de ser o comportamento anterior. Todo probe ja
    existente chama `scan(project)` e nao pode notar diferenca."""
    a = scanner_mod.ReadOnlyProjectScanner().scan(_arvore_de_tres())
    b = scanner_mod.ReadOnlyProjectScanner().scan(_arvore_de_tres(), visitor=None)
    assert json.dumps(a, sort_keys=True) == json.dumps(b, sort_keys=True)
    assert "visitor" not in a["tree"]


def test_visitante_e_chamado_uma_vez_por_no_alcancado():
    vistos = []
    ...
    r = scanner_mod.ReadOnlyProjectScanner().scan(_arvore_de_tres(),
                                                  visitor=visitante)
    assert sorted(vistos) == ["root", "root/0", "root/0/0", "root/1"]
    assert len(vistos) == len(set(vistos)), "nenhum no pode ser visitado duas vezes"
    assert r["statistics"]["total_nodes"] == len(vistos)
    assert r["tree"]["visitor"] == {"ok": True}


def test_visitante_recebe_o_objeto_VIVO_e_nao_o_registro_serializado():
    """A razao de o parametro existir: `hasattr` e `GetType()` so respondem no
    proxy, e o registro e serializavel justamente por nao carrega-lo."""
    tipos = []

    def visitante(proxy, record):
        tipos.append(hasattr(proxy, "get_children"))
        return None

    scanner_mod.ReadOnlyProjectScanner().scan(_arvore_de_tres(),
                                              visitor=visitante)
    assert all(tipos), "o visitante tem de receber o proxy, nao o dict"


def test_excecao_do_visitante_e_falha_de_RAMO_e_nao_derruba_o_scan():
    """Derrubar a varredura transformaria erro de classificacao em arvore
    ilegivel — dois diagnosticos distintos virando um."""
    def visitante(proxy, record):
        if record["node_id"] == "root/0":
            raise RuntimeError("classificador quebrou")
        return {"ok": True}

    r = scanner_mod.ReadOnlyProjectScanner().scan(_arvore_de_tres(),
                                                  visitor=visitante)
    assert r["statistics"]["scan_complete"] is True
    assert r["statistics"]["total_nodes"] == 4, "os outros nos continuam varridos"
    assert r["statistics"]["visitor_errors"] == 1
    assert any("visitante" in e["message"] for e in r["errors"])
    alvo = [c for c in r["tree"]["children"] if c["node_id"] == "root/0"][0]
    assert alvo["visitor"] == {"state": "visitor_failed"}
    assert alvo["identity"], "o no continua registrado apesar da falha"


def test_valor_nao_serializavel_e_descartado_com_diagnostico():
    """Um visitante que devolvesse o proxy vivo destruiria em silencio a
    propriedade que este scanner promete."""
    def visitante(proxy, record):
        return {"proxy": proxy}

    r = scanner_mod.ReadOnlyProjectScanner().scan(_arvore_de_tres(),
                                                  visitor=visitante)
    assert r["tree"]["visitor"] == {"state": "visitor_unserializable"}
    assert r["statistics"]["visitor_errors"] == 4
    json.dumps(r)  # a saida inteira continua serializavel


def test_visitante_nao_altera_o_percurso():
    """Ele nao escolhe filhos, nao poda e nao ordena: a varredura nao pode
    passar a depender do chamador."""
    def visitante(proxy, record):
        record["children"] = []      # tentativa de poda
        return {"tentou_podar": True}

    r = scanner_mod.ReadOnlyProjectScanner().scan(_arvore_de_tres(),
                                                  visitor=visitante)
    assert r["statistics"]["total_nodes"] == 4
