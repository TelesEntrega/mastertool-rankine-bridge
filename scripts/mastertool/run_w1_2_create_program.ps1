<#
.SYNOPSIS
    Host supervisionado de W1.2 -- criar UM PROGRAM ST vazio e persistir por
    save_as.
...
.DESCRIPTION
    FAIL-CLOSED. Tres modos MUTUAMENTE EXCLUSIVOS; o default nao muta nada:
        ...
        -PreflightOnly   abre o MasterTool e roda o probe 29 (preflight),
                         somente leitura. Prova create_program e o GUID de ST
        -ExecuteMutation validacao + preflight + probe 30 (mutacao) + probe 29
                         em modo postsave sobre o arquivo salvo

    O probe 30 e alcancavel APENAS pelo ramo -ExecuteMutation.
    ...
    secao 7 e poderia interromper gravacao de artefato. Se a janela nao fechar
    (tipicamente porque ha um dialogo aberto), o host REPORTA e nao insiste.
...
Write-Section "W1.2 -- criar PROGRAM ST -- modo '$Mode'"
...
if ($planData.phase -ne 'W1_2_CREATE_PROGRAM') { Fail "phase inesperada: $($planData.phase)" }
if ($planData.program_name -ne 'PRG_AI_TESTE') { Fail "program_name inesperado" }

$kinds = @($planData.operations | ForEach-Object { $_.kind })
if ($kinds.Count -ne 2 -or $kinds[0] -ne 'create_program' -or $kinds[1] -ne 'save_as') {
    Fail ("Plano deve declarar exatamente create_program, save_as -- recebido: " +
          ($kinds -join ', '))
}
Write-Host "[OK] Operacoes   : $($kinds -join ', ')"

$stGuid = $planData.st_language_guid
if ([string]::IsNullOrWhiteSpace($stGuid) -or $stGuid.Length -ne 36) {
    Fail "st_language_guid ausente ou sem forma de GUID: '$stGuid'"
}
Write-Host "[OK] GUID ST     : $stGuid"
...
$probe29 = Join-Path $RepoRoot 'scripts\mastertool\probes\29_preflight_program_w1_2_readonly.py'
if (-not (Test-Path -LiteralPath $probe29)) { Fail "Probe 29 nao encontrado" }
...
$preflightCompletion = Join-Path $preflightDir 'w1-2-preflight-completion.json'
$obs1 = Invoke-MasterTool -ProjectPath $copyPath -ProbePath $probe29 `
...
if ($null -eq $preflight) { Fail 'w1-2-preflight-completion.json ausente. Sem veredito.' }
...
if ($preflight.st_language_guid -ne $stGuid) {
    Fail ("GUID de ST medido ($($preflight.st_language_guid)) difere do plano ($stGuid). " +
          "O plano nao pode antecipar o que a medicao contradiz.")
}
...
if ($Mode -eq 'PreflightOnly') {
    Write-Host 'create_program disponivel, GUID de ST confirmado, nome livre.'
...
$probe30 = Join-Path $RepoRoot 'scripts\mastertool\probes\30_create_program_w1_2.py'
if (-not (Test-Path -LiteralPath $probe30)) { Fail "Probe 30 nao encontrado" }
...
$obs2 = Invoke-MasterTool -ProjectPath $copyPath -ProbePath $probe30 `
    -ScriptArgs ("--plan=" + $Plan) -StageDir $mutationDir -Label 'MUTACAO (probe 30)' `
...
Write-Host "[INFO] status do probe 30: $($completion.status)"
...
$postsaveCompletion = Join-Path $postsaveDir 'w1-2-postsave-completion.json'
$baselineTree = Join-Path $preflightDir 'w1-2-preflight-tree.json'
$obs3 = Invoke-MasterTool -ProjectPath $outputPath -ProbePath $probe29 `
...
if ($null -eq $postsave) { Fail 'w1-2-postsave-completion.json ausente.' }
...
$sessionReport = [ordered]@{
    ...
    created_declaration_sha256 = $completion.created_declaration_sha256
...
Write-Section "VEREDITO DA SESSAO W1.2: $verdict"
